...
export default function SuppliesPage({ params }: { params: Promise<{ store: string }> }) {
  ...
  const getUrgencyBadge = (urgency: UrgencyLevel) => {
    switch (urgency) {
      case 'Urgente':
        return <span className="flex items-center gap-1 bg-red-100 text-red-700 px-2 py-1 rounded-md text-[10px] font-bold uppercase"><AlertTriangle size={12} /> Urgente</span>;
      case 'Normal':
        return <span className="flex items-center gap-1 bg-blue-100 text-blue-700 px-2 py-1 rounded-md text-[10px] font-bold uppercase"><Info size={12} /> Normal</span>;
      default:
        return <span className="flex items-center gap-1 bg-slate-100 text-slate-600 px-2 py-1 rounded-md text-[10px] font-bold uppercase"><Clock size={12} /> Sem urgência</span>;
    }
  };
        ...
        <form onSubmit={handleAddOrder} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500 transition-all text-slate-800 font-medium appearance-none"
            >
          ...
          <button
            type="submit"
            disabled={adding}
            className="bg-red-600 hover:bg-red-700 text-white font-bold h-[50px] rounded-xl shadow-md shadow-red-100 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
          >
        ...
        {pendingOrders.length === 0 ? (
          ...
          <div className="grid gap-3">
            {pendingOrders.map((order) => (
              <div key={order.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between group hover:border-red-200 transition-all">
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex items-center gap-4 text-xs text-slate-400 font-medium">
                    {order.quantity && <span className="flex items-center gap-1">Qtd: {order.quantity}</span>}
                    <span className="flex items-center gap-1"><Calendar size={12} /> {order.createdAt?.toDate().toLocaleDateString('pt-BR')}</span>
                  </div>
                </div>
                <button
                  onClick={() => handleMarkAsDelivered(order.id)}
                  className="flex items-center gap-2 bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white px-4 py-2 rounded-lg font-bold text-sm transition-all border border-emerald-100"
                >
      ...
      <section className="space-y-4 pt-6 border-t border-slate-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 opacity-60 grayscale-[0.5]">
          {deliveredOrders.map((order) => (
            <div key={order.id} className="bg-slate-50 p-3 rounded-lg border border-slate-200 flex items-center justify-between">
              <div>
                <p className="text-md font-bold text-slate-700 line-through decoration-slate-400">{order.name}</p>
                <p className="text-[12px] text-slate-400 font-bold uppercase">
                  Marcado como entregue em: {order.deliveredAt?.toDate().toLocaleDateString('pt-BR')}
                </p>
              </div>
          ...
          {deliveredOrders.length === 0 && (
            <p className="text-sm text-slate-400 italic ml-1">Nenhuma entrega recente para exibir.</p>
          )}
        </div>
      </section>
    </div>
