  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen bg-slate-50">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm">
        ...
        {/* Navigation Tabs */}
        <nav className="max-w-5xl mx-auto px-4 flex gap-8">
          <TabLink 
            href={`/${store}/estoque`} 
...
function TabLink({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
  return (
    <Link 
      href={href}
      className="flex items-center gap-2 py-4 border-b-2 border-transparent hover:text-red-600 transition-all font-semibold text-slate-500"
